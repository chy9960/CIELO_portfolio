...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // 각 work 요소에 대해 클릭 이벤트를 추가합니다.
  works.forEach(work => {
      // work 요소 내 이미지 클릭 이벤트 처리
      work.addEventListener('click', function() {
          // data-instagram 속성에서 링크를 가져옵니다.
          const instagramLink = work.getAttribute('data-instagram');

          // 링크가 존재할 경우, 해당 링크로 이동합니다.
          if (instagramLink) {
              window.open(instagramLink, '_blank'); // 새 탭에서 링크 열기
          }
      });
  });
});
